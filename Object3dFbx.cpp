...
void Object3dFbx::Init()
{
	HRESULT result;
	//定数バッファの生成　トランフォーム行列用
	result = device->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
		D3D12_HEAP_FLAG_NONE,
		&CD3DX12_RESOURCE_DESC::Buffer((sizeof(ConstBufferDataTranceform) + 0xff) & ~0xff),
...
{	
	...
	camera->GetMatViewProjection();
	//モデルのメッシュトランスフォーム
	const XMMATRIX& modelTransform = modelFbx->GetModelTransform();
	//カメラ座標
	const XMFLOAT3& cameraPos = camera->GetEye();

	HRESULT result;
	//定数バッファへデータ転送
	ConstBufferDataTranceform* constMap = nullptr;
...
void Object3dFbx::Draw(ID3D12GraphicsCommandList* cmdList)
{
	...
	modelFbx->Draw(cmdList);
}

void Object3dFbx::PlayAnimation()
{
	FbxScene* fbxScene = modelFbx->GetFbxScene();
	//0番のアニメーション取得
	...
	//開始時間取得
	startTime = fbxTakeInfo->mLocalTimeSpan.GetStart();
	//終了時間取得
	endTime = fbxTakeInfo->mLocalTimeSpan.GetStop();
	//開始時間に合わせる
	currentTime = startTime;
	//再生中状態にする
	isPlay = true;
